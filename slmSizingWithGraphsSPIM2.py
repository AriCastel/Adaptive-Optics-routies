...
def evaluate_binary(slmShape, coverRange, steps, core):
    ...
    for i in tqdm(range(len(interval)), desc='Running thru the SLM', unit='Img'):
        ...
        #microData = takeImage()
        microData = np.zeros((10,10))# lumiere.take_Image(core)
        means.append(np.mean(microData))
        maxes.append(microData.max())

        current_datetime = datetime.now()
        folder_name = f"{current_datetime.strftime('%Y-%m-%d')}_Frames"
        os.makedirs(folder_name, exist_ok=True)
        image = Image.fromarray(microData.astype('uint16'))
        filename = f"frame_{i}.tiff"
