"""
Aberration Correction using Iterative process
By Artemis the Lynx
Version 2.0.20240517
"""
import aotools
import logging
import math
...
class adaptiveOpt:
    ...
    def metric_better_r(img, integration_radius=40):
    ...
    def threshold_matrix(matrix, percentile=99):
        """
        ...
        mini = int(thresholded_matrix.min())
        result = (thresholded_matrix - threshold_value )*(100/maxi)
...
def iterate(slmShape, fouriershape, centerpoint, stretch, degree, g_0, epsilon, totalIterations, slm, preview=True):
    """_summary_
    ...
    startimage = adaptiveOpt.get_guidestar(core,graph=preview)
    M0 = adaptiveOpt.metric_better_r(startimage)
    metric_t = M0
    ...
    for i in tqdm(range(totalIterations), desc="Optimizing", unit='iteration'): 
        ...
        metric_t = adaptiveOpt.metric_better_r(guideStar_t)
    ...
    if preview: 
        ...
        guideStar = adaptiveOpt.get_guidestar(core)
